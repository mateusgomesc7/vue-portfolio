<template>
  <div ref="card" class="webgl-container">
    <canvas class="webgl" :class="!overlay ? 'draggable' : ''"></canvas>
...
export default {
  ...
  methods: {
    ...
    createWorld() {
      ...
      const videoPortfolioTexture = this.getVideoTexture(
       ...
       */
      // Baked Material
      this.bakedLightMaterial = new MeshBasicMaterial({
        map: bakedLightTexture,
      });
      ...
      this.videoPortfolioMaterial = new MeshBasicMaterial({
        map: videoPortfolioTexture,
      });
      ...
      gltfLoader.load("computer.glb", (gltf) => {
        gltf.scene.traverse((child) => {
          if (this.$vuetify.theme.dark) child.material = this.bakedDarkMaterial;
          else child.material = this.bakedLightMaterial;
        });
    ...
    createBox() {
      ...
      if (this.$vuetify.theme.dark)
        this.box.material.color.set(this.$vuetify.theme.themes.light.primary);
      else this.box.material.color.set(0x616161);
    ...
    updateLights(mesh) {
      const alexaLightMesh = mesh.children.find(
        (child) => child.name === "alexaLight"
      );
      const fanLightAMesh = mesh.children.find(
        (child) => child.name === "fanLightA"
      );
      const fanLightBMesh = mesh.children.find(
        (child) => child.name === "fanLightB"
      );
      const symbolLightBMesh = mesh.children.find(
        (child) => child.name === "symbolLight"
      );
    ...
    updateScreens(mesh) {
      const screenBMesh = mesh.children.find(
        (child) => child.name === "screenB"
      );
      const screenAMesh = mesh.children.find(
        (child) => child.name === "screenA"
      );
